...
public class AltaController implements Initializable {
    ...
    public void darAlta(ActionEvent actionEvent) {
        ...
        if(tabAlumno.getSelectionModel().getSelectedItem() != null){
            ...
            try{
                int option = JOptionPane.showConfirmDialog(null,String.format("El siguiente usuario se dara de alta: %s con matricula: %s y rol de profesor",selected.getNomAlumno(),selected.getMatricula()));
            ...
            System.out.println(tabProf.getSelectionModel().getSelectedItem().toString());
        }
    }
